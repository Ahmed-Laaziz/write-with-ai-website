...
export default function Summarize({textToSummarize}){
    ...
    const [texttoSummarize, setTextToSummarize] = useState(textToSummarize);
    ...
    const handleCopyClick = () => {
        ...
        const fetchSummarization = async () => {
            ...
            try {
              ...
              const url = "http://localhost:8008/get_summarization"; // URL for the backend API
              const requestData = {
                text: textToSummarize, // Send the user input as a parameter in the request body
    ...
    return(
      ...
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop:"3%"}}>
        <Button
          ...
          disabled={isLoading}
        >
          <SummarizeIcon fontSize='small'/>&nbsp;Summarize
        </Button>
      </div>
    </ThemeProvider>
    <ThemeProvider theme={theme}>
...
Summarize.propTypes = {
    textToSummarize: PropTypes.string.isRequired,
  };